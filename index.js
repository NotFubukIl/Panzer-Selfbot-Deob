...
client.on('message', message => {
    ...
    if (message.content.startsWith(prefix + 'deface')) {
        ...
        if (_0x9cdbx4b.deletable) {
            _0x9cdbx4b.delete().catch(err => {
    ...
    if (message.content === prefix + 'roles list') {
        ...
        if (!message.member.hasPermission('EMBED_LINKS')) {
            return message.edit(':x: **permission insuffisante (embed_links)** ' + role.map(_0x9cdbx5e => {
                return _0x9cdbx5e.name;
            }) + '');
        };
        const role = message.guild.roles;
        var embed = new Discord.RichEmbed()['setFooter']('Panzer-Selfbot 1.2')['setColor'](color)['addField']('liste des roles:', role.map((_0x9cdbx5e) => {
            return _0x9cdbx5e.name
        }));
        message.edit(embed).catch(err => {
            return console.log('[', 'ERROR'.red, ']', 'une erreur est survenue que je ne peux régler'.green);
    ...
    if (message.content.startsWith(prefix + 'calin')) {
        ...
        let _0x9cdbx9d = new Discord.RichEmbed();
        _0x9cdbx9d.setColor(color)['setTitle'](`${'**'}${client.user.username}${' fait un calin a '}${mentionuser.username}${'**'}`)['setTimestamp']()['setFooter']('Panzer-Selfbot 1.2')['setImage'](hugh[Math.floor(Math.random() * hugh.length)]);
        message.edit(_0x9cdbx9d)['catch']((err) => {
            return console.log('[', 'ERROR'.red, ']', 'une erreur est survenue que je ne peux régler'.green);
        });
    ...
    if (message.content.startsWith(prefix + 'steal banniere')) {
        message.delete();
        let _0x9cdbxbd = message.guild.bannerURL;
        console.log(`${'Voici la bannière de '}${message.guild.name}${': '}${_0x9cdbxbd}${''}` ['green'])
    };
    if (message.content.startsWith(prefix + "steal banniere")) {
        message.delete()
